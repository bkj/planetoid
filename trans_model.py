...
class trans_model(base_model):
    ...
    def build(self):
        ...
        node_emb   = lasagne.layers.EmbeddingLayer(input_node, input_size=self.num_ver, output_size=self.embedding_size, W=source_emb.W)
        node_emb   = layers.DenseLayer(node_emb, self.y.shape[1], nonlinearity=lasagne.nonlinearities.softmax)
    ...
    def init_train(self, init_iter_label, init_iter_graph):
        ...
        for i in range(init_iter_label):
            ...
            print 'init_train: iter label', i, loss

        for i in range(init_iter_graph):
            gx, gy = next(self.graph_generator)
            loss = self.g_fn(gx, gy)
            print 'init_train: iter graph', i, loss
